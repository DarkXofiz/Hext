...
public class ElytraSwap extends BaseModule {
    ...
    public void onTick() {
        ...
        if (chest.getItem() == Items.ELYTRA) {
            if (state) {
                // Swap to hotbar slot 0 (chestplate)
                for (int i = 0; i < 9; i++) {
                    if (mc.player.getInventory().getStack(i).getItem() == Items.DIAMOND_CHESTPLATE ||
                        mc.player.getInventory().getStack(i).getItem() == Items.NETHERITE_CHESTPLATE) {

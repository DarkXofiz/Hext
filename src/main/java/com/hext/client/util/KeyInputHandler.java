...
public class KeyInputHandler {
    ...
    private void onTick(MinecraftClient client) {
        ...
        if (client.currentScreen == null) {
            ...
            if (freeLook != null && freeLook.isEnabled()) {
                boolean cPressed = GLFW.glfwGetKey(handle, GLFW.GLFW_KEY_C) == GLFW.GLFW_PRESS;
                if ("Hold".equals(freeLook.getMode())) {
                    freeLook.setActive(cPressed);
                }
            }
    ...
    private int getCurrentKey(long handle) {
        ...
        for (int key : keys) {
            if (GLFW.glfwGetKey(handle, key) == GLFW.GLFW_PRESS) return key;
        }
